...
if 0:
    filenames = './prepro_data' # refer root dir
    arr=[]
    eachFile(filenames)
    for i in arr:
        print (i)
    ...
    f=open("./tmp1+2_productsearch.json","r",encoding='utf-8')
    ddata = json.loads(f.read())
...
if 0:
    ...
    f2=open("./tmp1+2_invest_manager.json","w",encoding='utf-8')
    f2.write(json.dumps(dd,sort_keys=True,indent=4,ensure_ascii=False))
    # f.close()
    f2.close()
