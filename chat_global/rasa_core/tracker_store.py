...
import logging
from rasa_core.configs.read_config import read_redis_config
...
class RedisTrackerStore(TrackerStore):
    ...
    # modify fangning 20181114

    # def __init__(self, domain, host='172.23.16.29',
    #              port=6601, db='xdata-cache', password='redis1'):

    # modify wanghuan 20181123
    REDIS_CONF = read_redis_config()

    def __init__(self, domain, host=REDIS_CONF['host'], host2=REDIS_CONF['host2'],
                 port=REDIS_CONF['port'], db=REDIS_CONF['database'], password=REDIS_CONF['password']):    
        # import redis
        ...
        sentinel = Sentinel([(host,port),(host2,port)],socket_timeout=0.5)
        self.red = sentinel.master_for(service_name=db, socket_timeout=0.5, password=password)

        super(RedisTrackerStore, self).__init__(domain)

    def save(self, tracker, timeout=3600, pre='DLG:'):
        serialised_tracker = self.serialise_tracker(tracker)
        self.red.set(pre+str(tracker.sender_id), serialised_tracker, ex=timeout)
        # self.red.set(tracker.sender_id, serialised_tracker, ex=timeout)
    def retrieve(self, sender_id, pre='DLG:'):
        stored = self.red.get(pre+str(sender_id))
        # stored = self.red.get(sender_id)
        if stored is not None:
            return self.deserialise_tracker(str(sender_id), stored)
