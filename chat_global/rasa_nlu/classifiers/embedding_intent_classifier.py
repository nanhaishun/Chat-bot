...
from rasa_nlu.classifiers import Reg_IntentClassifier
...
class EmbeddingIntentClassifier(Component):
    ...
    def __init__(self,
        ...
        # fangning141 20181102
        self.process_intent_regex = self.component_config.get('intent_regex')
    ...
    def process(self, message, **kwargs):
        ...
        # modify by fangning 20181028
        if self.process_intent_regex:
            intent = Reg_IntentClassifier.reg_process(message)
            # print(intent)
            if intent["name"] is not None:
                message.set("intent", intent, add_to_output=True)                
                intent_ranking = []
                intent_ranking.append({"name":intent["name"],"confidence":1.0})
                for k,v in self.inv_intent_dict.items():
                    if v!= intent["name"] and len(intent_ranking)<INTENT_RANKING_LENGTH:
                        intent_ranking.append({"name":v,"confidence":0.0})
