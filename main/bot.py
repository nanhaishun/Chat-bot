...
class ActionSearchProduct(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if last_product_name != product_name:
            ...
            if last_product_information is None:
                results = self.search_product_info (dispatcher,tracker,domain,product_intent,last_product_intent,product_name,last_product_name,last_product_information)
                return [SlotSet(key,value) for (key,value) in results[0].items()]

            else:
                if product_intent in PRODUCT_INTENTS:
                    self.output_of_intent(dispatcher,tracker,domain,product_intent,last_product_information)
                    return [SlotSet("last_product_intent",product_intent),SlotSet("last_product_name",product_name)]
                elif last_product_intent in PRODUCT_INTENTS:
                    self.output_of_intent(dispatcher,tracker,domain,last_product_intent,last_product_information)
                    return [SlotSet("last_product_name",product_name)]  
                else:
                    dispatcher.utter_message("你想查询{%s}的什么信息呢？\n[投资范围|收益率|管理人|风险|期限|金额|募集时间|优惠]"%product_name)
                    return [SlotSet("last_product_information",last_product_information),SlotSet("last_product_name",product_name)]            
    ...
    def search_product_info (self,dispatcher,tracker,domain,product_intent,last_product_intent,product_name,last_product_name,last_product_information):
        ...
        for e in list_products_sorted:
            if e[1]>0.8:
                ...
                if product_intent in PRODUCT_INTENTS:
                    self.output_of_intent(dispatcher,tracker,domain,product_intent,res['data'])
                    # print("2222 product_intent:"+product_intent)
                    return [{"last_product_intent":product_intent,"last_product_information":res['data'],"last_product_name":product_name}]
                elif last_product_intent in PRODUCT_INTENTS:
                    self.output_of_intent(dispatcher,tracker,domain,last_product_intent,res['data'])
                    # print("3333 product_intent:"+product_intent)
                    return [{"last_product_information":res['data'],"last_product_name":product_name}]  
                else:
    ...
    def output_of_intent(self,dispatcher,tracker,domain,product_intent,product_information):
        try:
            if product_intent=='invest_scope' : #'投资范围':
